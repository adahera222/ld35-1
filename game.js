...
(function(){
  var ref$, map, filter, flatten, Key, FPS, RES, DIRS, remove, degToRad, Point, Stage, Layer, Centerable, Character, Critter, drawArc, drawCircle, drawSemicircle, drawTriangle, R, pick, randomColor, Behavior, Snuffle, Shrine, Blok, makeWall, isInWall, Map, FlowerStack, Flower, Room, Game;
  ...
  Room = (function(){
    ...
    function Room(x, y, width, height){
      var solid, empty, res$, i$, xi, lresult$, j$, yi, flowerSpots, fpos, this$ = this instanceof ctor$ ? this : new ctor$;
      width == null && (width = 20);
      ...
      this$.mapping = res$;
      flowerSpots = [Point(RES * 5, RES * 5), Point(RES * 5, RES * 10), Point(RES * 10, RES * 7), Point(RES * 15, RES * 5), Point(RES * 15, RES * 10)];
      fpos = pick(flowerSpots);
      this$.flower = new Character(fpos.x, fpos.y, RES, new FlowerStack);
      this$.flower.tags.push('flower');
      this$.flower.origin = Point(x, y);
      return this$;
    } function ctor$(){} ctor$.prototype = prototype;
    return Room;
  }());
  Game = (function(){
    ...
    function Game(width, height, fps){
      var i$, len$, worldSize, res$, to$, x, lresult$, j$, to1$, y, w2, base, solid, addWall, this$ = this;
      this.width = width;
      this.height = height;
      this.fps = fps;
      ...
      Key.bind('z', function(){
        ...
        this$.world.fg.add(cc);
        return window.critter = cc;
      });
      this.loopInstance = setInterval(this.loop, 1000 / this.fps);
      this.player = new Character(this.width / 2 - RES / 2, this.height / 2, RES, Snuffle('#aaa', '#000', '#aaa'));
      for (i$ = 0, len$ = DIRS.length; i$ < len$; ++i$) {
        (fn$.call(this, DIRS[i$]));
      }
      worldSize = Point(10, 3);
      res$ = [];
      for (i$ = 0, to$ = worldSize.x; i$ < to$; ++i$) {
        x = i$;
        lresult$ = [];
        for (j$ = 0, to1$ = worldSize.y; j$ < to1$; ++j$) {
          y = j$;
          lresult$.push(Room(x, y));
        }
        res$.push(lresult$);
      }
      this.worldMap = res$;
      w2 = ~~(worldSize / 2);
      base = this.worldMap[0][1];
      base.base = true;
      base.flower = null;
      base.flowers = [];
      base.shrine = new Character(this.width / 2 - 4 * RES, this.height / 2 + RES * 2, 8 * RES, Shrine('red', 'grey', 8 * RES));
      this.screen = Point(0, 1);
      this.loadRoom(this.worldMap[this.screen.x][this.screen.y]);
      this.world.fg.add(this.player);
      solid = ['black', 'black', 'white', true];
      addWall = curry$(function(side, grid){
        switch (side) {
        case 'left':
          grid[0] = repeatArray$([solid], grid[0].length);
          return grid;
        case 'right':
          grid[grid.length - 1] = repeatArray$([solid], grid[grid.length - 1].length);
          return grid;
        case 'top':
          return map(function(it){
            return it[0] = solid;
        ...
        case 'bottom':
          return map(function(it){
            return it[it.length - 1] = solid;
          }, grid);
        }
      });
      map(addWall('left'), map(function(it){
        return it.mapping;
      }, this.worldMap[0]));
      map(addWall('right'), map(function(it){
        return it.mapping;
      }, this.worldMap[this.worldMap.length - 1]));
      map(addWall('top'), map(function(it){
        return it[0].mapping;
      }, this.worldMap));
      map(addWall('bottom'), map(function(it){
        return it[it.length - 1].mapping;
      }, this.worldMap));
      this.player.addTick(function(){
        var step, grid, colors, getColor;
        step = 30;
        if (this$.frame % step > 0) {
          return;
        }
        grid = this$.world.bg.children[0].cells;
        colors = ['black', '#666', '#999', null];
        getColor = function(){
          return pick(colors);
        };
        map(function(it){
          return it.hour = getColor();
        }, grid[~~(this$.frame / step)]);
        map(function(it){
          return it.decade = getColor();
        }, grid[~~(this$.frame / step)]);
        return map(function(it){
          return it.instant = getColor();
        }, grid[~~(this$.frame / step)]);
      });
      this.player.addTick(function(){
        var flower, ref$;
        if (this$.worldMap[this$.screen.x][this$.screen.y] === base) {
          return;
        }
        flower = (ref$ = filter(function(it){
          return it.tagged('flower');
        }, this$.world.ground.children)) != null ? ref$[0] : void 8;
        if (flower && !this$.player.flower && this$.player.intersect(flower)) {
          this$.worldMap[this$.screen.x][this$.screen.y].flower = null;
          this$.player.flower = flower;
          this$.player.flower.graphics.scale(0.5);
          return flower.addTick(function(){
            flower.x = this$.player.x + this$.player.width / 2;
            return flower.y = this$.player.y;
          });
        }
      });
      function fn$(dd){
    ...
    prototype.screenSwap = function(){
      ...
      if (this.player.x + this.player.width > this.width) {
        this.screen = Point(this.screen.x + 1, this.screen.y);
        this.player.x = 1;
        this.loadRoom(this.worldMap[this.screen.x][this.screen.y]);
      }
      if (this.player.x < 0) {
        this.screen = Point(this.screen.x - 1, this.screen.y);
        this.player.x = this.width - this.player.width - 1;
        return this.loadRoom(this.worldMap[this.screen.x][this.screen.y]);
      }
    };
    ...
    prototype.loop = function(){
      ...
      this.screenSwap();
      this.world.ctx.save();
      this.world.ctx.translate(this.width / 2 - this.player.x, this.height / 2 - this.player.y);
      this.world.draw();
